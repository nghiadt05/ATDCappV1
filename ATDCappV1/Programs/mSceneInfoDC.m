function sceneInfo=mSceneInfoDC()
...
imgDir = opts.ImagesDir;
if ~exist('./tmp/mHumanTracking/demo','dir')
    mkdir('./tmp/mHumanTracking/demo');
end
dbfolder='./tmp/mHumanTracking/demo';

% sceneInfo.detfile=fullfile(dbfolder,'det','PETS2009-S2L1-c1-det.xml');
cDir = pwd;
cd(opts.DetFileTmpDir);
detFileDir = pwd;
sceneInfo.detfile = [detFileDir '\' opts.DetFileName];
cd(cDir);
sceneInfo.frameNums= opts.StartFrame:opts.EndFrame;
%sceneInfo.imgFolder=fullfile(dbfolder,'img',filesep);
sceneInfo.imgFolder = imgDir;
sceneInfo.imgFileFormat=opts.ImageForm;
...
%% ground truth
sceneInfo.gtFile='';
%sceneInfo.gtFile=fullfile(dbfolder,'gt','PETS2009-S2L1-c1.mat');
% sceneInfo.gtFile = 'D:\[2]DATA\[DATASET]\[1]VideoMining\TownCentre\gt\gtInfo.mat';
global gtInfo
